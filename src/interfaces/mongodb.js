...
db.test = () => {
    MongoClient.connect(url, options, (err, client) => {
        ...
        client.db(config.mongodb.db).collections()
            .then(collections => {
...
db.fetchAllFromCollection = collection => {
    return new Promise((resolve, reject) => {
        MongoClient.connect(url, options, (err, client) => {
            ...
            client.db(config.mongodb.db).collection(collection).find({}).toArray((error, result) => {
                if (error) reject(err);
...
db.fetchByIdFromCollection = (collection, id) => {
    return new Promise((resolve, reject) => {
        MongoClient.connect(url, options, (err, client) => {
            ...
            client.db(config.mongodb.db).collection(collection).findOne({
                "id": id
            }, (error, result) => {
                if (error) reject(err);
